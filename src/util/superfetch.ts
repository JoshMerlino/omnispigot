...
export default async function superfetch<T = Record<string, never>>(url: string, lifetime: number | null = 60*1000): Promise<T> {
	...
	// If store is expires
	if (store.age > (lifetime ?? 1e10) || JSON.stringify(store.value) === "{}") {
		...
		const resp = await fetch(url)
			...
			.catch(async() => {
				return await superfetch<T>(url, lifetime);
			});
